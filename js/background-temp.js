...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  if(message.injectOverlay == true){
    if (get_overlay_state(sender.tab.url)){
	    filename = "js/overlay.js";
	    inspire_update_check();
  		learn_now(function(learn_value) {
		    inspiration = get_inspirtion();
		    date = get_date();
		    color_theme = get_color_theme();
		    en_word = learn_value.english;
		    translated_word = learn_value.new_word;

		    saveCurrentInfo(translated_word,sender.tab.url);
		    set_last_run();
		    sendResponse({ done: true, color_to_add: color_theme, lang: get_lang(), en_word: en_word, new_word: translated_word, inspiration: inspiration, date: date  });

		});
	    return true; // Required for async sendResponse()
  	}
  } else if (message.getWord == true){//popup
	learn_now(function(learned_word) {
		date = get_date();
		color_theme = get_color_theme();
		...
		//1-add_to_word_analytics
		 ...
		 //2-saveLearningTime
   		...
   		clearInfo();
		sendResponse({ done: true, color_to_add: color_theme, lang: get_lang(), en_word: learn_word_en, new_word: learn_translated_word, date: date });
	 });
 	return true;
  }  else if (message.learnAnotherWordOverlay == true){
	learn_now(function(learned_word) {
		date = get_date();
		color_theme = get_color_theme();
	  	learn_word_en = learned_word.english;
 	...
 	return true;
  	...
  	return redirect();
  	 	...
  	 	sendResponse({response:check_redirect()})
   }
...
function redirect()
{
 	...
 	if(user.redirect_website)
	{
		...
		{
			chrome.tabs.update(tabs[0].id, {url: user.redirect_website});
		});
...
function learn_now(callback){
	var english_word = words_to_learn[Math.floor(Math.random()*words_to_learn.length)];
	word = null;
	...
	get_response(url, function(response) {
	  var matches = response.match(/\{([^}]+)\}/);
		...
		try {
			...
			r =  {"english": english_word, "new_word": result };
...
function learn_word(english_word){
	...
	get_response(url, function(response) {
		...
		try {
			result = (JSON.parse(submatch)).m;
			return_result = {"english": english_word, "new_word": result };
...
function add_to_blacklist(website){
	...
	if (item){
		...
		if (blacklist.includes(website_domain)) {
			return {status: false, message: "This website is already blacklisted."};
		}else{
			blacklist.push(website_domain);
			settings.blacklist = blacklist;
			storage_set("settings", settings);
			return {status: true, message: item};
		}
	}
	else {
		return {status: false, message: "Your input is not a valid website. Please copy paste the URL from Address bar."};
	}
...
function change_frequency(freq){
	var settings = storage_get("settings");
	if (freq == 0) {settings.always_run = true;} else { settings.always_run = false;}
