...
var randomZekrOverlayId = 'random-zekr-overlay';
var sabahZekrOverlayId = 'sabah-zekr-overlay';
var masaaZekrOverlayId = 'masaa-zekr-overlay';
...
//   document.getElementById("get-another-random-zekr").style.display = 'none';
//
//
// }
...
//   document.getElementById("get-another-random-zekr").style.display = 'inline';
// }

function injectZekrOverlay(overlayId, inner_code, color_to_add) {

  var overlay_block = document.getElementsByClassName(overlayId)[0];
  if (!overlay_block) {
    overlay_block = document.createElement('div');
    overlay_block.setAttribute("class", overlayId);
...
function remove_overlay(overlayId) {
  var element = document.getElementsByClassName(overlayId)[0];
  if (element) {
...
document.addEventListener('click', function(e) {
  if (!click_status && e.target) {
    if (e.target.id == 'get-another-random-zekr') {
      ...
      continueBrowsing();
      remove_overlay(randomZekrOverlayId);
      ...
      remove_overlay(sabahZekrOverlayId);
      ...
      remove_overlay(masaaZekrOverlayId);
...
// function injectZekrOverlay(overlayId, inner_code, color_to_add) {
//   injectZekrOverlay(overlayId, inner_code, color_to_add);
// }
...
if (!injectOverlayOff) {
  ...
  chrome.runtime.sendMessage({
    ...
    if (response && response.done) {
      ...
      var inner_code = `
            ...
            <img class="profpic" src="` + icon_src + `" alt="profile picture" />
						...
						<ul class="prof-sm">
							<li class="ks_sm" >
								<span title="Learn another word" >
									<svg id="get-another-random-zekr" class="svg-icon" viewBox="0 0 20 20">
					...
					<main class="ks-user-desc">
					  <p id="random-zekr" class="zekr-text">` + response.zekrData.zekr + `</p>
            <p id="random-fadl" class="zekr-text">` + response.zekrData.fadl + `</p>
					</main>
					<footer>
						<ul class="user-tags">
      ...
      injectZekrOverlay(randomZekrOverlayId, inner_code, response.color_to_add);
...
function showSabahZekr(message) {
  if (message.response && message.response.done) {
    ...
    <section class="profilebox">
        <aside class="ks_nomargin">
          <img class="profpic" src="` + icon_src + `" alt="profile picture" />
          <button class="prof-close" id="remove-sabah-overlay-btn" type="button"  >&times;</button>
          <ul class="prof-sm">
            <li class="ks_sm" >
              <span title="Learn another word" >
                <svg id="get-another-sabah-zekr" class="svg-icon" viewBox="0 0 20 20">
                  <path fill="none" d="M3.254,6.572c0.008,0.072,0.048,0.123,0.082,0.187c0.036,0.07,0.06,0.137,0.12,0.187C3.47,6.957,3.47,6.978,3.484,6.988c0.048,0.034,0.108,0.018,0.162,0.035c0.057,0.019,0.1,0.066,0.164,0.066c0.004,0,0.01,0,0.015,0l2.934-0.074c0.317-0.007,0.568-0.271,0.56-0.589C7.311,6.113,7.055,5.865,6.744,5.865c-0.005,0-0.01,0-0.015,0L5.074,5.907c2.146-2.118,5.604-2.634,7.971-1.007c2.775,1.912,3.48,5.726,1.57,8.501c-1.912,2.781-5.729,3.486-8.507,1.572c-0.259-0.18-0.618-0.119-0.799,0.146c-0.18,0.262-0.114,0.621,0.148,0.801c1.254,0.863,2.687,1.279,4.106,1.279c2.313,0,4.591-1.1,6.001-3.146c2.268-3.297,1.432-7.829-1.867-10.101c-2.781-1.913-6.816-1.36-9.351,1.058L4.309,3.567C4.303,3.252,4.036,3.069,3.72,3.007C3.402,3.015,3.151,3.279,3.16,3.597l0.075,2.932C3.234,6.547,3.251,6.556,3.254,6.572z"></path>
                </svg>
              </a>
            </li>
        ...
        <header class="ks_header">
          <h1 class="ks_center prof-name">اذكار الصباح</h1>
         </header>
        <main class="ks-user-desc">
          <p id="sabah-zekr" class="zekr-text">` + message.response.zekrData.zekr + `</p>
          <p id="sabah-fadl" class="zekr-text">` + message.response.zekrData.fadl + `</p>
          <p id="sabah-repetition" class="zekr-text"> مرة ` + message.response.zekrData.repetition + ` </p>
        </main>
        <footer>
          <ul class="user-tags">
        ...
        <row>
              ...
              <div class="col-sm-4">
                 <button class="ks_info" id="sabah-done-btn" type="button"   ">تمام</button>
          </div>
          <div class="col-sm-2">
                 <button class="ks_info" id="sabah-later-btn" type="button"   ">لاحقا</button>
          </div>
        </row>
        </container>
      </section>
  `;
    ...
    injectOverlayOff = true;
    injectZekrOverlay(sabahZekrOverlayId, inner_code, message.response.color_to_add);
    return true;
...
function showMasaaZekr(message) {
  if (message.response && message.response.done) {
    ...
    <section class="profilebox">
        <aside class="ks_nomargin">
          <img class="profpic" src="` + icon_src + `" alt="profile picture" />
          <button class="prof-close" id="remove-masaa-overlay-btn" type="button"  >&times;</button>
          <ul class="prof-sm">
            <li class="ks_sm" >
              <span title="Learn another word" >
                <svg id="get-another-masaa-zekr" class="svg-icon" viewBox="0 0 20 20">
                  <path fill="none" d="M3.254,6.572c0.008,0.072,0.048,0.123,0.082,0.187c0.036,0.07,0.06,0.137,0.12,0.187C3.47,6.957,3.47,6.978,3.484,6.988c0.048,0.034,0.108,0.018,0.162,0.035c0.057,0.019,0.1,0.066,0.164,0.066c0.004,0,0.01,0,0.015,0l2.934-0.074c0.317-0.007,0.568-0.271,0.56-0.589C7.311,6.113,7.055,5.865,6.744,5.865c-0.005,0-0.01,0-0.015,0L5.074,5.907c2.146-2.118,5.604-2.634,7.971-1.007c2.775,1.912,3.48,5.726,1.57,8.501c-1.912,2.781-5.729,3.486-8.507,1.572c-0.259-0.18-0.618-0.119-0.799,0.146c-0.18,0.262-0.114,0.621,0.148,0.801c1.254,0.863,2.687,1.279,4.106,1.279c2.313,0,4.591-1.1,6.001-3.146c2.268-3.297,1.432-7.829-1.867-10.101c-2.781-1.913-6.816-1.36-9.351,1.058L4.309,3.567C4.303,3.252,4.036,3.069,3.72,3.007C3.402,3.015,3.151,3.279,3.16,3.597l0.075,2.932C3.234,6.547,3.251,6.556,3.254,6.572z"></path>
                </svg>
              </a>
            </li>
        ...
        <header class="ks_header">
          <h1 class="ks_center prof-name">اذكار المساء</h1>
         </header>
        <main class="ks-user-desc">
          <p id="masaa-zekr" class="zekr-text">` + message.response.zekrData.zekr + `</p>
          <p id="masaa-fadl" class="zekr-text">` + message.response.zekrData.fadl + `</p>
          <p id="masaa-repetition" class="zekr-text"> مرة ` + message.response.zekrData.repetition + ` </p>
        </main>
        <footer>
          <ul class="user-tags">
            <li class="tag ks_uppercase" >` + message.response.higriDate + `</li>
        ...
        <row>
              ...
              <div class="col-sm-4">
                 <button class="ks_info" id="masaa-done-btn" type="button"   ">تمام</button>
          </div>
          <div class="col-sm-2">
                 <button class="ks_info" id="masaa-later-btn" type="button"   ">لاحقا</button>
          </div>
        </row>
        </container>
      </section>
  `;
    ...
    injectOverlayOff = true;
    injectZekrOverlay(masaaZekrOverlayId, inner_code, message.response.color_to_add);
    return true;
  }
}